...
Provides `base_logger(logger)` to ensure `logs/` exists and attach a
WARNING+ file handler at ``logs/price-calculator.log`` and a colored
console handler for DEBUG-level output. Does not add handlers if the
logger already has any.
"""
...
def base_logger(logger: logging.Logger) -> None:
    ...
    DEBUG-level stream handler. No handlers are added if the logger
